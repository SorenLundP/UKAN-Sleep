...
def train_student_model(student_model, data_loader, device, optimizer, scheduler, num_epochs=30000000):
    global train_loss, train_acc, train_kappa, train_f1, val_loss, val_acc, val_kappa, val_f1
    
    best_val_acc = 0.0
    no_improvement_epochs = 0
    log_file = open('UKAN_no_teacher_model_50%_training_log.txt', 'w')
    log_file.write('timestamp,epoch,train_loss,train_acc,train_kappa,train_f1,val_loss,val_acc,val_kappa,val_f1,lr,patience\n')
    log_file.close()
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_loader:
            ...
            student_probs = F.softmax(student_logits, dim=1)  # Add missing probability conversion
        ...
        if epoch % 2 == 0:  # Validate every 2 epochs
            ...
            with torch.no_grad():
                for val_batch in data_loader:
                    ...
                    batch_val_loss = F.cross_entropy(val_logits, val_labels.long())
                ...
                if acc > best_val_acc:
                    best_val_acc = acc
                    no_improvement_epochs = 0
                    torch.save(student_model.state_dict(), "UKAN_no_teacher_model_50%.pth")
        ...
        with open('UKAN_no_teacher_model_50%_training_log.txt', 'a') as log_file:
...
def save_plots():
    ...
    output_path = "./UKAN_no_teacher_model_50%_training_metrics.pkl"
    with open(output_path, 'wb') as f:
        pickle.dump(metrics, f)
        pickle.dump(fig, f)
    fig.savefig("training_metrics_plot_UKAN_no_teacher_model_50%.png")
...
def main():
    ...
    student_weights_path = "./UKAN_no_teacher_model_50%.pth"
    ...
    train_student_model(student_model, data_loader, device, optimizer, scheduler)
    
    # Save the final model
    torch.save(student_model.state_dict(), "final_UKAN_no_teacher_model_50%.pth")
    ...
    with open('training_metrics_lists_UKAN_no_teacher_model_50%.pkl', 'wb') as f:
        pickle.dump(metrics, f)
